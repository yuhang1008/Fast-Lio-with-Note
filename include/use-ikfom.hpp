...
// state_ikfom is 24 demension state
//    MTK_BUILD_MANIFOLD(state_ikfom,
//    ((vect3, pos))
//    ((SO3, rot))
//    ((SO3, offset_R_L_I))
...
Eigen::Matrix<double, 24, 1> get_f(state_ikfom &s, const input_ikfom &in)
{
	...
	vect3 omega;
	in.gyro.boxminus(omega, s.bg); // omaga saves gyro without bias
	vect3 a_inertial = s.rot * (in.acc-s.ba); // acc in world frame without bias

	// 0-2 velo(global), 3-5 omega, 12-14 acc in world
  for(int i = 0; i < 3; i++ ){
		res(i) = s.vel[i];
		res(i + 3) =  omega[i]; 
...
// 24 demension state and 2 demension imu input
// differential model. see fast-lio 1 eq.(7)
